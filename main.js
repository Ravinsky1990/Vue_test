Vue.component("data-list", {
    template: '<div class="button_wrap"> <button v-on:click="showdata()">{{btn_state}}</button> </div><input type="text" id="search" v-model="input" autofocus placeholder="Enter name, location or id"> <div class="sort_btn"> <button v-on:click="showSortByname()">Sort by name</button> <button v-on:click="showSortBylocation()">Sort by location</button> <button v-on:click="showSortBycurrensy()">Sort by currency</button> </div><div class="table_wrap"> <table v-show="show_ok"> <caption>Test Vue_js</caption> <tr> <th>id</th> <th>name</th> <th>location</th> <th>currency</th> </tr><tr v-for="item in customers | filterBy input"> <td>{{item.id}}</td><td>{{item.name}}</td><td>{{item.location}}</td><td>{{item.currency}}</td></tr><tr> <td class="total">Total</td><td></td><td></td><td class="total">{{total_cur}}</td></tr></table> </div>',
    data: function () {
        return {
            customers: [{
    ...
    computed: {
        total_cur: function () {
            ...
            this.customers.forEach(element => {
                total = total + element.currency
            });
            return total
        },
        sortByname: function () {
            let sort = this.customers
    ...
    methods: {
        showdata: function () {
            this.show_ok = !this.show_ok

            if (this.btn_state === "Show data") {
                this.btn_state = "Hide data"
            } else {
                this.btn_state = "Show data"
            }
        },

        showSortByname: function () {
            this.customers = this.sortByname
        },

        showSortBylocation: function () {
            this.customers = this.sortBylocation
        },
        showSortBycurrensy: function () {
            this.customers = this.sortBycurrency
        },
        classToogl: function () {
